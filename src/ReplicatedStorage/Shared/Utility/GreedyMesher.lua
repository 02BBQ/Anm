...
function Mesher:MergeNearby(part: Instance, OP: OverlapParams, mergeProperties: {}, Merged: {}, ResetId: string?)
	...
	for _, axisEnum in pairs(Enum.Axis:GetEnumItems()) do
		...
		for _, mult in pairs({ -1, 1 }) do
			...
			for i = 1, #touching do
				...
				part.CanTouch = false;
				if ResetId then
					part:SetAttribute('ResetId', ResetId);
				end;
				...
				self:MergeNearby(part, OP, mergeProperties, Merged, ResetId)
...
function Mesher:MergeParts(parts: {}, mergeProperties: {})
	...
	for _, part in pairs(parts) do
		self:MergeNearby(part, OP, mergeProperties, Meshed, part:GetAttribute('ResetId'))
	end
